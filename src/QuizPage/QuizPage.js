		...
		const results = (<div id="searchresults">
			<div className="search-result">
				<img className="item-image" src="https://imgnews.pstatic.net/image/022/2019/08/15/20190815508490_20190815184507880.jpg?type=w647" alt="search result"/>
					<div>
						<div className="item-title">文대통령 “평화경제에 모든 것 쏟아부을 것”</div>
						...
						<div className="sourceBox">
							...
							<Clue innerText="2019.08.15" tooltip="마스크를 쓰지 않은 사진은 2019년 행사 사진이네?" className="item-date" onClick={(event)=>{
			...
			<div className="search-result">
				...
				<div>
					<div className="item-title">문 대통령 내외, 제75주년 광복절 경축식 참석</div>
					...
					<div className="sourceBox">
						...
						onClick={()=>{
							this.onClickClue("올해 행사 사진과 불일치", -20);
				...
				<div>
					...
					<div className="sourceBox">
						<Clue innerText="대한민국 청와대 |" tooltip="청와대 공식 자료에서는 마스크를 쓴 모습이야." onClick={()=>{
							this.onClickClue("청와대 공식 자료와 불일치", -50);
			...
			<div id="quiz-wrapper">
				...
				<div id="quiz">
					...
					(<div className="quizContent" id="quiz1">
						<div id="quizitem-title">[일반] 문재인 등 <Clue innerText="8.15 행사" tooltip="올해 광복절 행사에서 진행된 일인가 보네." onClick={() => this.onClickKeyword("2020년 광복절 행사", 0)}/>에서 노마스크</div>
					...
					<div id="buttons">
						...
						{this.state.currentStep === 2?<button id="next-button" disabled={!this.state.factScoreChanged} onClick={()=>this.returnToTopics(clues.length)}>완료</button>:(null)}
						{this.state.currentStep < 2?<button id="next-button" disabled={!this.state.factScoreChanged && this.state.currentStep===1} onClick={()=>this.proceedStep()}>다음 단계</button>:(null)}
					</div>
